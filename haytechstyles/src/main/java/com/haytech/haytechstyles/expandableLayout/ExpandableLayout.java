...
public class ExpandableLayout extends ConstraintLayout {
    public interface State {
        int COLLAPSED = 0;   //جمع شده
        int COLLAPSING = 1;  //جمع آوری
        int EXPANDING = 2;  // گسترش
        int EXPANDED = 3;   // منبسط
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setVisibility(expansion == 0 && size == 0 ? View.INVISIBLE : View.VISIBLE);
        ...
        if (parallax > 0) {
            ...
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (orientation == HORIZONTAL) {
                    int direction = -1;
                    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN_MR1 && getLayoutDirection() == View.LAYOUT_DIRECTION_RTL) {
                        direction = 1;
                    }
    ...
    public void setExpansion(float expansion) {
        ...
        setVisibility(state == State.COLLAPSED ? View.GONE : View.VISIBLE);
